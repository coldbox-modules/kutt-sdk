	  ...
	  string domain
		...
		if( !urlResponse.isSuccess() ) {
			if( moduleSettings.throwOnError ){
				...
				log.error( "An error occurred while attempting to create the short URL: #urlResponse.getStatus()# #urlResponse.getStatusText()#", urlResponse.getMemento() );
				return target;
			}
		} else {
