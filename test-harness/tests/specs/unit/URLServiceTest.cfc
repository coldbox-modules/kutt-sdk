component extends="coldbox.system.testing.BaseTestCase"{
	...
	function afterAll(){
		if( !isNull( variables.model ) ){
			variables.model.list( all=true ).data.each( ( link ) => {
	...
	function run(){
		describe( "Link CRUD", function(){
			...
			it( "Can update a tiny url", function(){
				...
				expect( dateDiff( "d", now(), newLink.expire_in ) )
						.toBeLT( 2 );
				...
				expect( dateDiff( "d", now(), updatedLink.expire_in ) )
						.toBeLT( 31 );
			});
